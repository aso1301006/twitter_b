...
$tue_week1 =explode(",", $_GET['con_tue_week1']);
$count=0;
while($count<24){
	$ydata[]=$tue_week1[$count];
	$count++;
}

$tue_week2 =explode(",", $_GET['con_tue_week2']);
$count=0;
while($count<24){
	$ydata2[]=$tue_week2[$count];
	$count++;
}

$tue_week3 =explode(",", $_GET['con_tue_week3']);
$count=0;
while($count<24){
	$ydata3[]=$tue_week3[$count];
	$count++;
}

$tue_week4 =explode(",", $_GET['con_tue_week4']);
 $count=0;
 while($count<24){
 	$ydata4[]=$tue_week4[$count];
 	$count++;
 }
 ...
 if($last_day>=funcDesignatedDay($n, $w)){
 	...
 	$tue_week5 =explode(",", $_GET['con_tue_week5']);
 	$count=0;
 	while($count<24){
 		$ydata5[]=$tue_week5[$count];
 		$count++;
 	}
...
//グラフのタイトル設定
 $title = mb_convert_encoding("火曜日", "UTF-8", "auto");
