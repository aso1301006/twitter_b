...
$sun_week1 =explode(",", $_GET['con_sun_week1']);
$count=0;
while($count<24){
	$ydata[]=$sun_week1[$count];
	$count++;
}

$sun_week2 =explode(",", $_GET['con_sun_week2']);
$count=0;
while($count<24){
	$ydata2[]=$sun_week2[$count];
	$count++;
}

$sun_week3 =explode(",", $_GET['con_sun_week3']);
$count=0;
while($count<24){
	$ydata3[]=$sun_week3[$count];
	$count++;
}

$sun_week4 =explode(",", $_GET['con_sun_week4']);
 $count=0;
 while($count<24){
 	$ydata4[]=$sun_week4[$count];
 	$count++;
 }


//$ydata5=GET['sun_week5'];
...
// // Create the graph. These two calls are always required
 ...
 $title = mb_convert_encoding("日曜日", "UTF-8", "auto");
...
$graph->Add($lineplot2);
 ...
 //if($sun_count==5){もし5週目が見つからない（一か月の範囲内のうちデータが4つしかない）場合、5本目をプロットしない
 //$graph->Add($lineplot5);
 //}
