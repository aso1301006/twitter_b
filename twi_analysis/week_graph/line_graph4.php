...
$thu_week1 =explode(",", $_GET['con_thu_week1']);
$count=0;
while($count<24){
	$ydata[]=$thu_week1[$count];
	$count++;
}

$thu_week2 =explode(",", $_GET['con_thu_week2']);
$count=0;
while($count<24){
	$ydata2[]=$thu_week2[$count];
	$count++;
}

$thu_week3 =explode(",", $_GET['con_thu_week3']);
$count=0;
while($count<24){
	$ydata3[]=$thu_week3[$count];
	$count++;
}

$thu_week4 =explode(",", $_GET['con_thu_week4']);
 $count=0;
 while($count<24){
 	$ydata4[]=$thu_week4[$count];
 	$count++;
 }


//$ydata5=GET['thu_week5'];
...
$graph->Add($lineplot2);
 ...
 //if($thu_count==5){もし5週目が見つからない（一か月の範囲内のうちデータが4つしかない）場合、5本目をプロットしない
 //$graph->Add($lineplot5);
 //}
