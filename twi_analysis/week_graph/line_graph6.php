...
$sat_week1 =explode(",", $_GET['con_sat_week1']);
$count=0;
while($count<24){
	$ydata[]=$sat_week1[$count];
	$count++;
}

$sat_week2 =explode(",", $_GET['con_sat_week2']);
$count=0;
while($count<24){
	$ydata2[]=$sat_week2[$count];
	$count++;
}

$sat_week3 =explode(",", $_GET['con_sat_week3']);
$count=0;
while($count<24){
	$ydata3[]=$sat_week3[$count];
	$count++;
}

$sat_week4 =explode(",", $_GET['con_sat_week4']);
 $count=0;
 while($count<24){
 	$ydata4[]=$sat_week4[$count];
 	$count++;
 }


//$ydata5=GET['sat_week5'];
...
// // Create the graph. These two calls are always required
 ...
 $title = mb_convert_encoding("土曜日", "UTF-8", "auto");
...
$graph->Add($lineplot2);
 ...
 //if($sat_count==5){もし5週目が見つからない（一か月の範囲内のうちデータが4つしかない）場合、5本目をプロットしない
 //$graph->Add($lineplot5);
 //}
